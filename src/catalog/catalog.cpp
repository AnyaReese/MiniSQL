...
CatalogManager::CatalogManager(BufferPoolManager *buffer_pool_manager, LockManager *lock_manager, LogManager *log_manager, bool init)
  ...
  if(!init)//如果不是初次创建
  {
    ...
    for(auto it : catalog_meta_->index_meta_pages_){//获取所有索引的元信息的目录
      ...
      index_names_[tables_[index_meta->GetTableId()]->GetTableName()][index_meta->GetIndexName()] = index_meta->GetIndexId();//index_names_[表名][索引名] = index_id
...
dberr_t CatalogManager::CreateTable(const string &table_name, TableSchema *schema, Txn *txn, TableInfo *&table_info) {
  try{
    ...
    table_heap_=table_heap_->Create(buffer_pool_manager_,schema_, txn,log_manager_,lock_manager_);//初始化table_heap
...
dberr_t CatalogManager::GetIndex(const std::string &table_name, const std::string &index_name,
  ...
  if(idx_name2id.find(index_name)==idx_name2id.end())
